...
bool isValidUsername(const std::string& username) {
  // Username must not be empty and can only contain alphanumeric characters and
  // underscores
  return (username.length() > 0) &&
         (username.find_first_not_of(VALID_USERNAME_CHARACTER) ==
          std::string::npos);
}
...
void addUser(const User& user) {
  ...
  if (!isValidUsername(user.username)) {
    Serial.println(
        "[UserAuth::addUser] Username is not supported. Please only use the "
        "following "
        "characters: "
...
User searchUser(const std::string& username) {
  ...
  logVerboseln(
      ("[UserAuth::searchUser] Searching for user: " + username).c_str());
  File users =
      LittleFS.open(filename, "r");  // Programm seems to crash at this point
  logVerboseln(
  ...
  while (users.available()) {
    ...
    if (user.username == username) {
      ...
      if (commaIndex2 == std::string::npos) {
        logVerboseln(
            ("[UserAuth::searchUser] Parse old format of: " + line).c_str());
        user.passwordHash =
            line.substr(commaIndex1 + 1, line.length() - commaIndex1 - 1);
        size_t endIndex = user.passwordHash.find('\r');
        user.passwordHash = user.passwordHash.substr(0, endIndex);
        user.role = UserRole::User;
      } else {
        logVerboseln(
            ("[UserAuth::searchUser] Parse new format of: " + line).c_str());
        user.passwordHash =
            line.substr(commaIndex1 + 1, commaIndex2 - commaIndex1 - 1);
        user.role =
            line.substr(commaIndex2 + 1, line.length() - commaIndex2 - 1);
...
bool checkPasswordHash(
    const User& user,
    const std::string& passwordHash) {  // Replace String with std::string
  logVerboseln(
      ("[UserAuth::checkPasswordHash] Checking password hash for user " +
       user.username)
          .c_str());
  logVerboseln(("[UserAuth::checkPasswordHash]  User password hash: \"" +
                user.passwordHash + "\"")
                   .c_str());
  logVerboseln(("[UserAuth::checkPasswordHash] Input password hash: \"" +
                passwordHash + "\"")
                   .c_str());
...
bool checkAccess(const std::string& username, const std::string& password) {
  ...
  if (user.username == "" || user.passwordHash == "") {
    logAuthln(
        ("[UserAuth::checkAcces] User " + username + " not found").c_str());
    return false;
  }
  if (checkPasswordHash(user, hashPassword(password))) {
    logAuthln(
        ("[UserAuth::checkAcces] User " + username + " authenticated").c_str());
    return true;
  }
  logAuthln(
      ("[UserAuth::checkAcces] User " + username + " typed wrong password")
          .c_str());
  return false;
}
...
bool isAdmin(const User& user) {
  logVerboseln(
      ("[UserAuth::isAdmin] User role: \"" + user.role + "\"").c_str());
  logVerboseln(
      ("[UserAuth::isAdmin] Admin role: \"" + UserRole::Admin + "\"").c_str());
...
bool checkAdminAccess(const std::string& username,
                      const std::string& password) {
  logAuthln("[UserAuth::checkAdminAccess] begin authentication");
  User user = searchUser(username);

  if (user.username == "" || user.passwordHash == "") {
    logAuthln(("[UserAuth::checkAdminAccess] User " + username + " not found")
                  .c_str());
    return false;
  }
  logAuthln(
      ("[UserAuth::checkAdminAccess] User " + username + " found").c_str());
  if (!checkPasswordHash(user, hashPassword(password))) {
    logAuthln(("[UserAuth::checkAdminAccess] User " + username +
               " typed wrong password")
                  .c_str());
    return false;
  }
  logAuthln(("[UserAuth::checkAdminAccess] User " + username + " authenticated")
                .c_str());
  if (!isAdmin(user)) {
    logAuthln(
        ("[UserAuth::checkAdminAccess] User " + username + " is not an admin")
            .c_str());
    return false;
  }
  logAuthln(("[UserAuth::checkAdminAccess] User " + username + " is an admin")
                .c_str());
  return true;
}
